...
export default function AnalyticsDashboard() {
  ...
  // ✅ FIX: use the correct storage key
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#0d0d14] flex items-center justify-center transition-colors duration-300">
        <div className="text-indigo-600 dark:text-indigo-400 text-xl animate-pulse">
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#0d0d14] flex items-center justify-center transition-colors duration-300">
        <div className="text-red-500 dark:text-red-400 text-lg text-center px-6">
          {error}
        </div>
      </div>
    );
  ...
  // ✅ FIX: guard against empty hours array
  const maxHour =
    hours.length > 0
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#0d0d14] dark:text-white px-6 py-10 transition-colors duration-300">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-slate-500 dark:text-white/40 mt-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-10">
          {[
            ...
            <div
              key={card.label}
              className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-4 text-center shadow-sm dark:shadow-none"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-sm dark:shadow-none">
            <h2 className="text-lg font-semibold mb-4 text-indigo-600 dark:text-indigo-300">
              🎭 Genre Distribution
            </h2>
            {genres.length === 0 ? (
              <p className="text-slate-400 dark:text-white/30 text-center py-8">
                No genre data yet
              </p>
            ) : (
                ...
                <PieChart>
                  ...
                  <Tooltip
                    contentStyle={{
                      ...
                      border: isDark ? "1px solid #ffffff20" : "1px solid #e2e8f0",
                      borderRadius: 8,
          ...
          {/* Productive Hours */}
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 shadow-sm dark:shadow-none">
            <h2 className="text-lg font-semibold mb-1 text-indigo-600 dark:text-indigo-300">
            ...
            {hours.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={250}>
                ...
                >
                  <CartesianGrid strokeDasharray="3 3" stroke={isDark ? "#ffffff10" : "#cbd5e1"} />
                  <XAxis
                    dataKey="label"
                    tick={{ fill: isDark ? "#ffffff40" : "#64748b", fontSize: 10 }}
                    interval={3}
                  />
                  <YAxis tick={{ fill: isDark ? "#ffffff40" : "#64748b", fontSize: 10 }} />
                  <Tooltip
                    contentStyle={{
                      background: isDark ? "#1a1a2e" : "#ffffff",
                      border: isDark ? "1px solid #ffffff20" : "1px solid #e2e8f0",
            ...
            <div className="overflow-x-auto">
              <div className="flex gap-1 flex-wrap">
                {Array.from({ length: 365 }, (_, i) => {
                  ...
                  const opacity =
                    count === 0 ? 0.05 : count === 1 ? 0.3 : count === 2 ? 0.6 : 1;
                  return (
                    <div
                      key={dateStr}
